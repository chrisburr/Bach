...
bool TbPlotTool::initialize(AlgVec  algos) {
  ...
  tbroot->AddHisto1D("track_slopemc_slope_xz","track_slopemc_slope_xz",75,0.,1.5);
  tbroot->AddHisto1D("track_slopemc_slope_yz","track_slopemc_slope_yz",75,0.,1.5);
  ...
  for (std::map<std::string,TbModule*>::iterator itr = m_geomSvc->Modules.begin();
    ...
    tbroot->AddHisto3D(("Residuals2D_x_afteralign"+(*itr).first).c_str(),("Residuals2D_x_afteralign"+(*itr).first).c_str(),32,-7.04,7.04,32,-7.04,7.04);
    tbroot->AddHisto3D(("Residuals2D_y_afteralign"+(*itr).first).c_str(),("Residuals2D_y_afteralign"+(*itr).first).c_str(),32,-7.04,7.04,32,-7.04,7.04);
    ...
    tbroot->AddHisto1D(("Cluster_real-recox_1hit"+(*itr).first).c_str(),("Cluster_real-recox_1hit"+(*itr).first).c_str(),120,-1.2,1.2);
    tbroot->AddHisto1D(("Cluster_real-recoy_1hit"+(*itr).first).c_str(),("Cluster_real-recoy_1hit"+(*itr).first).c_str(),120,-1.2,1.2);
    tbroot->AddHisto1D(("Cluster_real-recox_2hit"+(*itr).first).c_str(),("Cluster_real-recox_2hit"+(*itr).first).c_str(),120,-1.2,1.2);
    tbroot->AddHisto1D(("Cluster_real-recoy_2hit"+(*itr).first).c_str(),("Cluster_real-recoy_2hit"+(*itr).first).c_str(),120,-1.2,1.2);
    tbroot->AddHisto1D(("Cluster_real-recox_3hit"+(*itr).first).c_str(),("Cluster_real-recox_3hit"+(*itr).first).c_str(),120,-1.2,1.2);
    tbroot->AddHisto1D(("Cluster_real-recoy_3hit"+(*itr).first).c_str(),("Cluster_real-recoy_3hit"+(*itr).first).c_str(),120,-1.2,1.2);
}
...
bool TbPlotTool::execute(AlgVec algos) {
  ...
  for (it = tracks->begin(); it != tracks->end(); ++it) {
    ...
    for (ic = clusters->begin(); ic != clusters->end(); ++ic) {
      ...
      tbroot->Histo3D("Residuals2D_x_"+(*ic)->id())->Fill((*ic)->GlobalPos().X(),(*ic)->GlobalPos().Y(),res_x);
      tbroot->Histo3D("Residuals2D_y_"+(*ic)->id())->Fill((*ic)->GlobalPos().X(),(*ic)->GlobalPos().Y(),res_y);
...
bool TbPlotTool::finalize() {
  if (TbAlignment* align = dynamic_cast<TbAlignment*> (find(m_algos,"TbAlignment")) ){
	...
	tbroot->Histo3D("Residuals2D_x_afteralign"+(*ic)->id())->Fill((*ic)->GlobalPos().X(),(*ic)->GlobalPos().Y(),res_x);
