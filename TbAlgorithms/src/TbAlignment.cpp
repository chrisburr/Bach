...
bool TbAlignment::finalize() {  
  ...
  bool DOF[6] = {1,1,0,1,1,1};      //DoF to align           
  double Sigm[6] = {0.1,0.1,0.005,0.001,0.001,0.08};  //Sigma for each DoF
  ...
  for (int i = 0; i <  Const_I("Iterations"); ++i){ 
    ...
    if (Cons[3] && DOF[1])   m_millepede->ConstF(&sheary[0],  -(nglo)*(m_slopey));//+1.40022e-04));     
    if (Cons[4] && DOF[2])   m_millepede->ConstF(&ftz[0],     0.0);       
    if (Cons[5] && DOF[2])   m_millepede->ConstF(&fscaz[0],   0.0); 
    if (Cons[6] && DOF[3])   m_millepede->ConstF(&frotx[0],   0.0);//-m_slopey);   
    if (Cons[7] && DOF[4])   m_millepede->ConstF(&froty[0],   0.0);//-m_slopex);       
    ...
    for (int i=0;i<n_fits; ++i) {
      ...
      PutTrack(m_trackcontainer->at(i), nglo, nloc, DOF);
...
bool  TbAlignment::PutTrack( TbTrack *track, int nglo, int nloc, bool m_DOF[])
{
  ...
  for(; itclus != cluster_sort->end(); ++itclus) {
    ...
    if (m_DOF[3]) derNonLin_i[3*Nmodules+n_station] =  3.0;      // d_alpha 1.0
    if (m_DOF[4]) derNonLin_i[4*Nmodules+n_station] =  3.0;      // d_beta 1.0
    ...
    if (m_DOF[3]) derNonLin_i[3*Nmodules+n_station] =  3.0;      // d_alpha 3.0
    if (m_DOF[4]) derNonLin_i[4*Nmodules+n_station] =  3.0;      // d_beta 3.0
    if (m_DOF[5]) derNonLin_i[5*Nmodules+n_station] =  0.0;      // d_gamma
    sc = m_millepede->EquLoc(&derGB[0], &derLC[0], &derNonLin[0], &derNonLin_i[0],
