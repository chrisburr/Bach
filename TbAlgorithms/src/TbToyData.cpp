 ...
 *
 *  Class to generate tracks, determine interception points and store corresponding hits
 *
 */
...
bool TbToyData::execute(AlgVec algos) {
  ...
  for (int j = 0; j < Const_I("NoOfTracks") ; ++j){
    ...
    for(it_type iterator = align.begin(); iterator != align.end(); iterator++) {
	 ...
	 m_hits -> push_back(hita);
	 ++hitidnr;
	 float mid_y = (pix_y_out-pix_y_in)/(pix_x_out-pix_x_in)*(ref_x-pix_x_in)+pix_y_in;
	 float mid_x = (pix_x_out-pix_x_in)/(pix_y_out-pix_y_in)*(ref_y-pix_y_in)+pix_x_in;
