...
bool Millepede::ParSig(int index, double sigma) {
  ...
  if (m_debug)
    cout << "--> Parsig " << index << " value " << psigm->at(index) << endl;
  return true;
}
...
bool Millepede::EquLoc(double dergb[], double derlc[], double dernl[],
  ...
  if (m_debug)
    cout << "dergl first and last index: " << iagb << " / " << ibgb << endl;
...
bool Millepede::FitLoc(int n, double track_params[], int single_fit) {
  ...
  if (itert < 2 && single_fit != 1)  // Do it only once //hier!!
  {
    ...
    for (i = 0; i < nst; i++)  // Store the track parameters
    {
      ...
      // if (arenl->at(i) != 0.) arest->at(i) = 0.0; // Reset global derivatives
      // if non linear and first iteration
    }
  ...
  /*
    ...
    with previous alignment constants for example (set with PARGLO). Also if
    there
  ...
  */
  ...
  if (m_debug)
    cout << " Printout of local fit  (FITLOC)  with rank= " << nrank << endl;
  if (m_debug)
  ...
  while (ist <= nst) {
    if (indst->at(ist) == -1) {
      ...
      else  // Third  0 : end of equation
      {
        ...
        if (m_debug)
          cout << "Number of derivatives (global, local): " << ndergl << ", "
               << nderlc << endl;
        if (m_debug)
          cout << "Global derivatives are: (index/derivative/parvalue) "
               << endl;

        for (i = (jb + 1); i < ist; i++) {
          if (m_debug)
            cout << indst->at(i) << " / " << arest->at(i) << " / "
                 << pparm->at(indst->at(i)) << endl;
        }
        if (m_debug)
          cout << "Global Non-Lin derivatives are: (index/derivative/parvalue) "
               << endl;
...
bool Millepede::SpAX(std::vector<std::vector<double>> *a,
...
bool Millepede::SpAVAt(std::vector<std::vector<double>> *v,
...
bool Millepede::MakeGlobalFit(std::vector<double> *par,
  ...
  while (itert < itelim)  // Iteration for the final loop
  {
    ...
    for (i = 0; i < nagb; i++) {
      if (psigm->at(i) <= 0.0)  // fixed global param
      {
        ...
        (cgmat->at(i))[i] += 1.0 / (psigm->at(i) * psigm->at(i));
      }
    }
    ...
    if (itert > 1) {
      for (j = 0; j < nagb; j++) {
        ...
        if (m_debug)
          cout << "diag. value : " << j << " = " << std::setprecision(5)
               << (cgmat->at(j))[j] << endl;
        ...
        for (i = 0; i < nagb; i++) {
          if (psigm->at(i) > 0.0) {
            ...
            if (i == j)
              final_cor -= step[i] * step[i] / (psigm->at(i) * psigm->at(i));
          }
        }
      }
    }
    ...
    for (i = 0; i < nagb; i++) {
      ...
      step[i] = bgvec->at(i);
      //      if (bgvec[i] < psigm[i]/10.)  psigm[i] = -1.;  // Fix parameter if
    ...
    for (int s = 0; s < storeare->size(); ++s) {
      if (storeind->at(s) == -1 && storeare->at(s) < 62500)
        //      std::cout << dparm->at(k) << "    " << storeare->at(s) << "   "
        //      <<storeind->at(s)<<"   " <<endl;
  ...
  for (j = 0; j < nagb; j++) {
    ...
    dparm->at(j) = 0.;
    pull->at(j) =
        par->at(j) / sqrt(psigm->at(j) * psigm->at(j) - (cgmat->at(j))[j]);
    error->at(j) = sqrt(fabs((cgmat->at(j))[j]));
  }

  cout << std::setw(10) << " " << endl;
  cout << std::setw(10) << "            * o o                   o      "
       << endl;
  cout << std::setw(10) << "              o o                   o      "
       << endl;
  cout << std::setw(10) << "   o ooooo  o o o  oo  ooo   oo   ooo  oo  "
       << endl;
  cout << std::setw(10) << "    o  o  o o o o o  o o  o o  o o  o o  o "
       << endl;
  cout << std::setw(10) << "    o  o  o o o o oooo o  o oooo o  o oooo "
       << endl;
  cout << std::setw(10) << "    o  o  o o o o o    ooo  o    o  o o    "
       << endl;
  cout << std::setw(10) << "    o  o  o o o o  oo  o     oo   ooo  oo ++ ends."
       << endl;
  cout << std::setw(10) << "                       o                   "
       << endl;
...
bool Millepede::PrtGlo() {
  ...
  for (int i = 0; i < nagb; i++) {
    ...
    if (fabs((cgmat->at(i))[i] * diag->at(i)) > 0) {
      ...
      cout << std::setw(4) << i << "  / " << std::setw(10) << pparm->at(i)
           ...
           << std::setw(13) << dparm->at(i) << "  / " << std::setw(13)
           << bgvec->at(i) << "  / " << std::setw(10) << std::setprecision(5)
           << err << "  / " << std::setw(10) << gcor << endl;
    } else {
  ...
  for (int i = 0; i < nagb; i++) {
    if (m_debug)
      cout << " i=" << i
           << "  sqrt(fabs(cgmat[i][i]))=" << sqrt(fabs((cgmat->at(i))[i]))
           << " diag = " << diag->at(i) << endl;
  }
