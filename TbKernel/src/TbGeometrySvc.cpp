...
bool TbGeometrySvc::readConditions() {
  ...
  if (!infile) {
    std::cerr << "Can't open input file " << Const_S("GeometryFile")
              << std::endl;
    exit(1);
  } else
    std::cout << "Open input file " << Const_S("GeometryFile") << std::endl;
  std::string line;
  while (!infile.eof()) {
    ...
    Modules[chip]->SetAlignment(chip, dx, dy, dz, rx, ry, rz, 0., 0., 0., 0.,
                                0., 0.);
  }
...
bool TbGeometrySvc::readConditionsXML() {
  ...
  BOOST_FOREACH (const ptree::value_type &a1, geometry) {
    ...
    Modules[chip] = new TbModule();
    Modules[chip]->SetAlignment(chip, x, y, z, rx, ry, rz, dx, dy, dz, drx, dry,
                                drz);
  }
  return true;
}
...
void TbGeometrySvc::writeConditionsXML(std::string outfile) {
  ...
  for (; itm != Modules.end(); ++itm) {
    myfile << "\t<Module name=\'" << (*itm).first << "\' X=\'"
           << (*itm).second->X() << "\' Y=\'" << (*itm).second->Y() << "\' Z=\'"
           << (*itm).second->Z() << "\' RX=\'" << (*itm).second->RotX()
           << "\' RY=\'" << (*itm).second->RotY() << "\' RZ=\'"
           << (*itm).second->RotZ() << "\' dX=\'" << (*itm).second->dX()
           << "\' dY=\'" << (*itm).second->dY() << "\' dZ=\'"
           << (*itm).second->dZ() << "\' dRX=\'" << (*itm).second->dRotX()
           << "\' dRY=\'" << (*itm).second->dRotY() << "\' dRZ=\'"
           << (*itm).second->dRotZ() << "\' />" << std::endl;
  }
