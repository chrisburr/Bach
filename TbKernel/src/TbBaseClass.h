...
class TbBaseClass
{
 public:
  ...
  virtual bool initialize(std::vector< std::pair< std::string, TbBaseClass* > >){return true;}
  virtual bool execute(std::vector< std::pair< std::string, TbBaseClass* > >){return true;}
...
return (*iter).second;}
  ...
  void PrintAlgorithm()                             {for (std::map<std::string,std::string>::const_iterator iter = m_const_s.begin();
